...
처음 연결에서 `DevToolsActivePort` 오류가 나오면 `open_chrome_devtools_setup` 도구가 Chrome 설정 페이지와 1688 홈을 열어줍니다. 열린 Chrome 화면에서 DevTools 연결을 허용한 뒤 새 채팅에서 다시 요청하면 됩니다.
...
| `auto` | Chrome DevTools 기반 실사용 흐름 안내 | Chrome DevTools 연결 |

Chrome DevTools 경로가 기본입니다. 별도 Playwright 창을 반복해서 여는 방식보다, 사용자가 보고 있는 Chrome 페이지와 네트워크를 읽는 흐름을 우선합니다.
