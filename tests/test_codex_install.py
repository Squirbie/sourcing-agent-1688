...
def write_marketplace_bundle(home: Path) -> None:
    ...
    (manifest_dir / "plugin.json").write_text(json.dumps({"version": "0.5.14"}), encoding="utf-8")
...
def test_install_codex_registers_marketplace_plugin_and_removes_global_mcp(monkeypatch, tmp_path):
    ...
    assert (tmp_path / "plugins" / "cache" / codex_install.MARKETPLACE_NAME / codex_install.PLUGIN_NAME / "0.5.14").exists()
